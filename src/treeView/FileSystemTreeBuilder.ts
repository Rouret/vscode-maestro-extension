...
import { IconType, getIconPathByIconType } from '../utils/RessourceManager';
...
export function getFolderTreeFromParentPath(parentPath: string): ItemTree {
    ...
    if (stats.isDirectory()) {
        const children = fs.readdirSync(parentPath)
            ...
            .filter(fullChildPath => {
                ...
                return childStats.isDirectory() || path.extname(fullChildPath) === '.yaml';
            })
            .map(validChildPath => {
